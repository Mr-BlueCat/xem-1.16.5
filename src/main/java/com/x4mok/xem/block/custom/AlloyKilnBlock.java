...
import com.x4mok.xem.tileentity.AlloyKilnTile;
...
public class AlloyKilnBlock extends Block {
    ...
    public ActionResultType use(BlockState blockState, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult rayTraceResult) {
        if(!world.isClientSide) {
            ...
            if(!player.isCrouching()) {
                if (tileEntity instanceof AlloyKilnTile) {
    ...
    private INamedContainerProvider createContainerProvider(World world, BlockPos pos) {
        return new INamedContainerProvider() {
            ...
            public ITextComponent getDisplayName() {
                return new TranslationTextComponent("screen.xem.alloykiln");
            }

            @Nullable
            @Override
            public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity player) {
                return new AlloyKilnContainer(i, world, pos, playerInventory, player);
            }
        };
    }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntities.ALLOY_KILN.get().create();
    }
