...
public class AlloyKilnContainer extends Container {
    private final TileEntity tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;

    public AlloyKilnContainer(int windowId, World world, BlockPos pos,
                              PlayerInventory playerInventory, PlayerEntity player) {

        super(ModContainers.ALLOY_KILN_CONTAINER.get(), windowId);
        ...
        if(tileEntity != null) {
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                ...
                addSlot(new SlotItemHandler(h, 1, 68, 18));
                addSlot(new SlotItemHandler(h, 2, 49, 54));
                addSlot(new SlotItemHandler(h, 3, 130, 38));
            });
        }
    }
